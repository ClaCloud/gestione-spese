...
function Card(props) {
  return (
    <div className="carta">
      <span>
        Saldo
        <div data-metod={props.id} className="imp visi-metod"><i className={`fas fa-eye${props.visibile == 0 ? '-slash' : ''}`}></i><span className="hover">{props.visibile == 1 ? 'Nascondi' : 'Aggiungi'} al totale</span></div>
      </span>
      <CurrencyFormat value={props.soldi} isNumericString={true} displayType={'text'} thousandSeparator={'.'} decimalSeparator={','} decimalScale={2} fixedDecimalScale={true} renderText={value =>
        <span className="totale">€ {value}</span>
      } />
      <span className="nome_carta">
        {props.metodo}
        <a data-modal={`metodo`} data-id={props.id} data-metodo={props.metodo} data-totale={props.soldi} className="imp open-modal"><i className="fas fa-cog"></i></a>
      </span>
    </div>
  )
}
...
function Profilo() {
  ...
  const [modalMetodo, setmodalMetodo] = useState({ id: "", metodo: "", totale: "" });
  ...
  const pagination = {
    ...
    renderBullet: function (index, className) {
      return `<h3 class="selection ${className}">${index === 0 ? "Modifica" : "Elimina"}</h3>`
    }
  };
  ...
  return (
    <div id="profilo" className={itemsLoaded ? (null) : ('preloading')}>
      <Modale
        dataModale="metodo"
        content={
          <Swiper pagination={pagination} autoHeight={true} spaceBetween={20} className="modmetSlider">
            <SwiperSlide>
              <form onSubmit={ModMet} style={{ paddingBottom: "0px" }}>
                ...
                <div className="row no-wrap">
                  <div className="col-2">
                    <button type="submit" className="button mini">Conferma</button>
                  </div>
                  <div className="col-2">
                    <a data-modal="metodo" className="button mini bgalert close-modal">Annulla</a>
                  </div>
                </div>
              </form>
            </SwiperSlide>
              ...
              <SwiperSlide hidden={modalMetodo.id === 1 ? true : false}>
                <form onSubmit={ElMet} style={{ paddingBottom: "0px" }}>
                  <div>
                    <div className="row">
                      <div className="col">
                        <p>Confermi di voler eliminare <b>{modalMetodo.metodo}</b>?</p>
                      </div>
                    </div>
                  </div>
                  <div className="row no-wrap">
      ...
      <div className="top">
        ...
        <div className="totmetodi">
          <Swiper slidesPerView={'auto'} centeredSlides={true} spaceBetween={10} className="mySwiper">
            {metodi.map(card => (
              <SwiperSlide>
                <Card key={card.id} id={card.id} visibile={card.visibile} metodo={card.metodo} soldi={card.totale} />
              </SwiperSlide>
            ))}
            <SwiperSlide>
              <div className="carta">
                <div className="aggiungi">
                  <span className="icona"><i className="fas fa-plus"></i></span>
                  <span className="testo">Aggiungi un nuovo metodo di pagamento</span>
                </div>
      ...
      <div className="modal">
        <div className="container">
          <div className="init row no-wrap">
            ...
            <div className="col-3">
              <a href="/logout.php" className="button mini bgalert"><span>Logout</span> <i className="fas fa-sign-out-alt"></i></a>
            </div>
          </div>
          ...
          <Box link={`/profilo/categorie`} icona={'/assets/img/icons/cartella.png'} motivo={'Modifica Categorie'} nonData={`Gestisci liberamente le tue categorie`} nonPrezzo={<i className="fas fa-chevron-right"></i>} />

          <Box link={`/profilo/rec-metodo`} icona={'/assets/img/icons/icons8-credit-card-100.png'} motivo={'Recupero Metodi'} nonData={`Recupera Metodi di Pagamento eliminati`} nonPrezzo={<i className="fas fa-chevron-right"></i>} />

          <Box link={`/profilo/stats-utente`} icona={'/assets/img/search.png'} motivo={'Statistiche Utente'} nonData={`Controlla tutti i dati raccolti sul tuo account`} nonPrezzo={<i className="fas fa-chevron-right"></i>} />

          <Box link={`/profilo/password`} icona={'/assets/img/password.png'} motivo={'Modifica Password'} nonPrezzo={<i className="fas fa-chevron-right"></i>} />
